C         Last modification on April 1 2019 by Lina Alasfar
C ==================================================================
C ================= PROGRAM (Light flavour) HDECAY: COMMENTS =======================
C ==================================================================
C
C                       *****************
C                       * VERSION 6.52 X  *
C                       *****************
C WARNING ::
C  MODIFIED BY LINA ALASFAR TO INCLUDE LIGHT FLAVOUR WITH MODIFIED YUKAWA COUPLINGS
C  Please note that the ncorrections are only written to the ordinary output
C   not the Les houches accourd . The light quarks masses have no running for now.
...
C Cd:      RESCALING FACTOR OF HDD COUPLING
C Cu:      RESCALING FACTOR OF HUU COUPLING
...
c changed Lina
      COMMON/MASSES_HDEC/AMS,AMC,AMB,AMT,AMD,AMUP
c end changed Lina
     ...
     .               SMBRGA,SMBRZGA,SMBRW,SMBRZ,SMWDTH,SMBRU,SMBRD
...
C CHANGED LINA
     ...
     .                 CPGAGA,CPGG,CPZGA,ICOUPELW,CPU,CPD
C END CHANGED LINA
...
c---------------------------------------------------
      READ(NI,100)ALSMZ
c  changed Lina 18.04.19 -- light flavours
      READ(NI,100)AMUP
      READ(NI,100)AMD
c  end changed Lina
      READ(NI,100)AMS
...
c ----------------------------- c
 ...
 105  format('#')
      ...
      IF(IHIGGS.EQ.0)THEN
      WRITE(NSA,20)AMSM,SMBRB,SMBRL,SMBRM,SMBRS,SMBRC,SMBRT,SMBRU,SMBRD
...
C =====================================================================
     ...
     .           CAS,CAU,CAD,CTS,CTU,CTD,CLU,CLS,CLD
     ...
     .           CXT10,CXT20, CAS0,CAU0,CAD0
      COMPLEX*16 CLE
      COMPLEX*16 CTTP,CTBP,CTEP,CLTP,CLBP,CLEP,CATP,CABP,CAEP
      COMPLEX*16 CATP0,CABP0
      COMPLEX*16 CAT00,CAB00,CAC00,CAL00,CAW00,CAS00,CAD00,CAU00
...
c end MMM changed 21/8/13
     ...
     .            *(1.D0-4.D0*Y+12.D0*Y**2)
      ...
      ! CHANGED LINA
            HFFM(X,Z,Y)= GF/(4*PI*DSQRT(2.D0))*X**3*Z*(BETA_HDEC(Y))**3
      ! END CHANGED LINA
...
C CHANGED LINA
       CTS = 4*AMS**2/AMH**2*DCMPLX(1D0,-EPS)
       CTU = 4*AMUP**2/AMH**2*DCMPLX(1D0,-EPS)
       CTD = 4*AMD**2/AMH**2*DCMPLX(1D0,-EPS)
C END CHANGED LINA
       CAT0= 2*CTT*(1+(1-CTT)*CF(CTT))
       CAB0= 2*CTB*(1+(1-CTB)*CF(CTB))
       CAC0= 2*CTC*(1+(1-CTC)*CF(CTC))
C CHANGED LINA
       CAS0= 2*CTS*(1+(1-CTS)*CF(CTS))
       CAU0= 2*CTU*(1+(1-CTU)*CF(CTU))
       CAD0= 2*CTD*(1+(1-CTD)*CF(CTD))
C  END CHANGED LINA
       CAT = CAT0 * CPT
       CAB = CAB0 * CPB
       CAC = CAC0 * CPC
...
C CHANGED LINA
       XFAC = CDABS(CAT+CAB+CAC+CAS+CAU+CAD+CATP+CABP)**2*FQCD
     .      + 4*DREAL(DCONJG(CAT+CAB+CAC+CAS+CAU+CAD+CATP+CABP)*CPGG)*DQCD
     .      + (2*CPGG)**2*PQCD
C  END  CHANGED LINA
C  mass dependent NLO QCD corrections
...
C  H ---> G G* ---> G CC   TO BE ADDED TO H ---> CC
       ...
       PQCD=PHGGQCD(ASG,NFEXT)
C CHANGED LINA
       XFAC = CDABS(CAT+CAB+CAC+CAS+CAD+CAU+CATP+CABP)**2*FQCD
     .      + 4*DREAL(DCONJG(CAT+CAB+CAC+CATP+CABP)*CPGG)*DQCD
     .      + (2*CPGG)**2*PQCD
C END  CHANGED LINA
C  mass dependent NLO QCD corrections  !LINA :: NO NEED TO INCLUDE LIGHT FLAVOUR HERE
       XFAC0 =(CDABS(CAT)**2*(XHGG(1)+XHQQ(1)*NFEXT)
     .       + CDABS(CAB)**2*(XHGG(2)+XHQQ(2)*NFEXT)
     .       + 2*DREAL(DCONJG(CAT)*CAB)*(XHGG(3)+XHQQ(3)*NFEXT))*ASG/PI
       XFAC = XFAC + XFAC0
...
C CHANGED LINA
       XFAC = CDABS(CAT+CAB+CAC+CAS+CAU+CAD+CATP+CABP)**2*FQCD
     .      + 4*DREAL(DCONJG(CAT+CAB+CAC+CAS+CAD+CAU+CATP+CABP)*CPGG)*DQCD
     .      + (2*CPGG)**2*PQCD
C END  CHANGED LINA
C  mass dependent NLO QCD corrections
       XFAC0 =(CDABS(CAT)**2*(XHGG(1)+XHQQ(1)*NFEXT)
...
c        XFAC00 = CDABS(CAT0+CAB0+CAC0+CATP0+CABP0)**2*FQCD
         XFAC00= DREAL(DCONJG(CAT+CAB+CAC+CAS+CAU+CAD+CATP+CABP)
     .                     * (CAT0+CAB0+CAC0+CATP0+CABP0))*FQCD
        ELSE
         XFAC00 = CDABS(CAT+CAB+CAC+CAS+CAD+CAU+CATP+CABP)**2*FQCD
     .          + 4*DREAL(DCONJG(CAT+CAB+CAC+CATP+CABP)*CPGG)*DQCD
     .          + (2*CPGG)**2*PQCD
        ENDIF
...
C  H ---> E E
       CPEL = CPMU
       AMELEC = 0.510998910D-3
       IF(AMH.LE.2*AMELEC) THEN
        HEE = 0
       ELSE
       HEE=HFF(AMH,(AMELEC/AMH)**2)
     .    *(1+ELW0(AMH,AMELEC,-1.D0,7.D0))
     .    *(1+ELW(AMH,AMELEC,0.D0,-1.D0,-1/2.D0))
     .    *HFFSELF(AMH)
     .    *SM4FACF
     .    * CPEL**2
      IF(ICOUPELW.EQ.0)THEN
       HEE=HFF(AMH,(AMELEC/AMH)**2) * CPMU
     .    * ((CPEL-1)
     .       +(1+ELW0(AMH,AMELEC,-1.D0,7.D0))
     .       +(1+ELW(AMH,AMELEC,0.D0,-1.D0,-1/2.D0))
     .       *HFFSELF(AMH) * SM4FACF)
...
C  H --> UU
      ...
      ELSE
       ...
       RATCOUP = CPT/CPU
      ENDIF
      XQCD1= QCDH(RMU**2/AMH**2)
      XQCD = (XQCD0-XQCD1)/2/XQCD1
      IF(AMH.LE.2*AMUP) THEN
       HUU = 0
      ELSE
       HU2=3.D0*HFFM(AMH,(RMU/AMH)**2,0.D00)
     .    *QCDH(RMU**2/AMH**2)
     .    *(1+ELW(AMH,AMUP,AMC,2/3.D0,1/2.D0))
     .    *HFFSELF(AMH) * SM4FACF
     .    * CPU**2
      IF(ICOUPELW.EQ.0)THEN
       HU2=3.D0*HFFM(AMH,(RMU/AMH)**2,0.D00) * CPU
     .    *QCDH(RMU**2/AMH**2)
     .    * ((CPU-1)
     .         +(1+ELW(AMH,AMUP,AMC,2/3.D0,1/2.D0)*(1+XQCD))
     .       *HFFSELF(AMH) * SM4FACF)
      ENDIF

       HU1=3.D0*HFFM(AMH,(RMU/AMH)**2,0.D00)
     .    *TQCDH(AMUP**2/AMH**2)
c    .    *(1+ELW0(AMH,RMS,-1.D0/3.D0,7.D0))
     ...
     .    *CPU**2
      IF(ICOUPELW.EQ.0)THEN
       HU1=3.D0*HFFM(AMH,(RMU/AMH)**2,0.D00)* CPU
     .    *TQCDH(AMUP**2/AMH**2)
     .    * ((CPU-1)
     .         +(1+ELW(AMH,AMUP,AMC,2/3.D0,1/2.D0))
     .       *HFFSELF(AMH) * SM4FACF)
...
C  H --> DD
      ...
      ELSE
       ...
       RATCOUP = CPT/CPD
      ENDIF
      XQCD1= QCDH(RMD**2/AMH**2)
      XQCD = (XQCD0-XQCD1)/2/XQCD1
      IF(AMH.LE.2*AMD) THEN
       HDD = 0
      ELSE
       HD2=3.D0*HFFM(AMH,(AMD/AMH)**2,0.D0)
     .    *QCDH(RMD**2/AMH**2)
c    .    *(1+ELW0(AMH,RMS,-1.D0/3.D0,7.D0))
     .    *(1+ELW(AMH,AMD,AMC,-1/3.D0,-1/2.D0))
     .    *HFFSELF(AMH) * SM4FACF
     .    * CPD**2
      IF(ICOUPELW.EQ.0)THEN
       HD2=3.D0*HFFM(AMH,(AMD/AMH)**2,0.0D0 )* CPD
     .    *QCDH(RMD**2/AMH**2)
     .    * ((CPD-1)
c    .         +(1+ELW0(AMH,RMS,-1.D0/3.D0,7.D0)*(1+XQCD))
     .         +(1+ELW(AMH,AMD,AMC,-1/3.D0,-1/2.D0)*(1+XQCD))
     .       *HFFSELF(AMH) * SM4FACF)
      ENDIF
c      IF(HS2.LT.0.D0) HS2 = 0
       HD1=3.D0*HFFM(AMH,(AMD/AMH)**2,0.0D0 )
     .    *TQCDH(AMD**2/AMH**2)
c    .    *(1+ELW0(AMH,RMS,-1.D0/3.D0,7.D0))
     .    *(1+ELW(AMH,AMD,AMC,-1/3.D0,-1/2.D0))
     .    *HFFSELF(AMH) * SM4FACF
     .    *CPD**2
      IF(ICOUPELW.EQ.0)THEN
       HD1=3.D0*HFFM(AMH,(AMD/AMH)**2,0.0D0 ) * CPD
     .    *TQCDH(AMD**2/AMH**2)
     .    * ((CPD-1)
c    .         +(1+ELW0(AMH,RMS,-1.D0/3.D0,7.D0))
     .         +(1+ELW(AMH,AMD,AMC,-1/3.D0,-1/2.D0))
...
C  H --> SS
      ...
      IF(AMH.LE.2*AMS) THEN
       HSS = 0
      ELSE
       HS2=3.D0*HFFM(AMH,(AMS/AMH)**2,(RMS/AMH)**2)
     .    *QCDH(RMS**2/AMH**2)
c    .    *(1+ELW0(AMH,RMS,-1.D0/3.D0,7.D0))
     ...
     .    * CPS**2
      IF(ICOUPELW.EQ.0)THEN
       HS2=3.D0*HFFM(AMH,(AMS/AMH)**2,(RMS/AMH)**2) * CPS
     .    *QCDH(RMS**2/AMH**2)
     .    * ((CPS-1)
c    .         +(1+ELW0(AMH,RMS,-1.D0/3.D0,7.D0)*(1+XQCD))
...
C  H --> CC
      ...
      IF(AMH.LE.2*AMC) THEN
       HCC = 0
      ELSE
       HC2=3.D0*HFFM(AMH,(AMC/AMH)**2,(RMC/AMH)**2)
     .    *QCDH(RMC**2/AMH**2)
c    .    *(1+ELW0(AMH,RMC,2.D0/3.D0,7.D0))
     ...
     .   + DCC
      IF(ICOUPELW.EQ.0)THEN
       HC2=3.D0*HFFM(AMH,(AMC/AMH)**2,(RMC/AMH)**2) * CPC
     .    *QCDH(RMC**2/AMH**2)
     .    * ((CPC-1)
...
C  H --> BB :
      ...
      IF(AMH.LE.2*AMB) THEN
       HBB = 0
      ELSE
       HB2=3.D0*HFFM(AMH,(AMB/AMH)**2,(RMB/AMH)**2)
     .    *QCDH(RMB**2/AMH**2)
c    .    *(1+ELW0(AMH,RMB,-1.D0/3.D0,1.D0))
     ...
     .   + DBB
       IF(ICOUPELW.EQ.0)THEN
        HB2=3.D0*HFFM(AMH,(AMB/AMH)**2,(RMB/AMH)**2 )* CPB
     .     *QCDH(RMB**2/AMH**2)
     .     * ((CPB-1)
...
C  H ---> GAMMA GAMMA
       EPS=1.D-8
C CHANGED LINA
       XRMS = AMS !RUNM_HDEC(AMH/2,4)*AMS/RUNM_HDEC(AMC,4)
       XRMU =  AMUP !RUNM_HDEC(AMH/2,4)*AMUP/RUNM_HDEC(AMC,4)
       XRMD =AMD ! RUNM_HDEC(AMH/2,4)*AMD/RUNM_HDEC(AMC,4)
C END CHANGED LINA
...
C CHANGED LINA
       CTS = 4*XRMS**2/AMH**2*DCMPLX(1D0,-EPS)
       CTU = 4*XRMU**2/AMH**2*DCMPLX(1D0,-EPS)
       CTD = 4*XRMD**2/AMH**2*DCMPLX(1D0,-EPS)
C  END CHANGED LINA
...
C CHANGED LINA
       CAS00= 1/3D0 * 2*CTS*(1+(1-CTS)*CF(CTS))
       CAU00= 4/3D0 * 2*CTU*(1+(1-CTU)*CF(CTU))
       CAD00= 1/3D0 * 2*CTD*(1+(1-CTD)*CF(CTD))
C END CHANGED LINA
...
C CHANGED LINA
       CAS0= 1/3D0 * 2*CTS*(1+(1-CTS)*CF(CTS)) * CPS
       CAU0= 4/3D0 * 2*CTU*(1+(1-CTU)*CF(CTU)) * CPU
       CAD0= 1/3D0 * 2*CTD*(1+(1-CTD)*CF(CTD)) * CPD
C
     ...
     .     * CFACQ_HDEC(0,AMH,XRMC)
      CAS = 1/3D0 * 2*CTS*(1+(1-CTS)*CF(CTS)) * CPS
     .     * CFACQ_HDEC(0,AMH,XRMS)
      CAU = 4/3D0 * 2*CTU*(1+(1-CTU)*CF(CTU)) * CPU
     .     * CFACQ_HDEC(0,AMH,XRMU)
      CAD = 1/3D0 * 2*CTD*(1+(1-CTD)*CF(CTD)) * CPD
     .     * CFACQ_HDEC(0,AMH,XRMD)
C END CHANGED LINA
       ...
       CABP00 = 0
       CAEP00 = 0
       XFAC0= CDABS(CAT0+CAB0+CAC0+CAS0+CAD0+CAU0+CAL+CAW+CPGAGA)**2
...
c       XFAC0= CDABS(CAT00+CAB00+CAC00+CAL00+CAW00)**2*GAGA_ELW(AMT,AMH)
        XFAC0= DREAL(DCONJG(CAT0+CAB0+CAC0+CAS0+CAD0+CAU0+CAL+CAW+CPGAGA)
     .       * (CAT00+CAB00+CAC00+CAS00+CAD00+CAU00CAL00+CAW00))
     .       * IOELW*GAGA_ELW(AMT,AMH)
       ENDIF
       IF(ISM4.NE.0)THEN
...
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
     ...
     .       - 237*AMTP**2/10 - 117*AMBP**2/10 - 3*XXQ)
        ...
        XFAC0= CDABS(CAT0+CAB0+CAC0+CAS0+CAD0+CAU0+CAL+CAW+CATP0+CABP0+CAEP+CPGAGA)**2
     .       * ((1+CELW)**2 - 1)
       IF(ICOUPELW.EQ.0)THEN
        XFAC0= DREAL(DCONJG(CAT0+CAB0+CAC0+CAS0+CAD0+CAU0+CAL+CAW+CATP0+CABP0+CAEP
     .                     +CPGAGA)
     .       * (CAT00+CAB00+CAC00+CAS00+CAD00+CAU00+CAL00+CAW00+CATP00+CABP00+CAEP00))
     .       * ((1+CELW)**2 - 1)
       ENDIF
...
c       write(6,*)CDABS(CAW)**2,CDABS(CAT+CAB+CAC+CAL+CAW)**2
       ENDIF
       XFAC = CDABS(CAT+CAB+CAC+CAS+CAD+
     . CAU+CAL+CAW+CATP+CABP+CAEP+CPGAGA)**2
       HGA=HVV(AMH,0.D0)*(ALPH/PI)**2/16.D0*(XFAC+XFAC0)
              PRINT *,  'H -> GAMMA GAMMA ' , HGA
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
C CHANGED LINA
C       XRMS = AMS !RUNM_HDEC(AMH/2,4)*AMS/RUNM_HDEC(AMC,4)
C       XRMU =  AMUP !RUNM_HDEC(AMH/2,4)*AMUP/RUNM_HDEC(AMC,4)
C       XRMD =AMD ! RUNM_HDEC(AMH/2,4)*AMD/RUNM_HDEC(AMC,4)
C END CHANGED LINA
      XRMC = RUNM_HDEC(AMH/2,4)*AMC/RUNM_HDEC(AMC,4)
      XRMB = RUNM_HDEC(AMH/2,5)*AMB/RUNM_HDEC(AMB,5)
      XRMT = RUNM_HDEC(AMH/2,6)*AMT/RUNM_HDEC(AMT,6)
      ...
      ELSE
       ...
       ! CHANGED LINA
              FS = 3*2D0/3*(-1+4*2D0/3*SS)/DSQRT(SS*CS) * CPS
              FD = 3*2D0/3*(-1+4*2D0/3*SS)/DSQRT(SS*CS) * CPD
              FU = -3*2D0/3*(1-4*2D0/3*SS)/DSQRT(SS*CS) * CPU
       ! END LINA CHANGED
...
c      CTC = 4*XRMC**2/AMH**2*DCMPLX(1D0,-EPS)
       ...
       CTC = 4*AMC**2/AMH**2*DCMPLX(1D0,-EPS)
       ! CHANGED LINA
C            CTS = 4*AMS**2/AMH**2*DCMPLX(1D0,-EPS)
C            CTU = 4*AMU**2/AMH**2*DCMPLX(1D0,-EPS)
C            CTD = 4*AMD**2/AMH**2*DCMPLX(1D0,-EPS)
       ! END LINA CHANGED
       CTL = 4*AMTAU**2/AMH**2*DCMPLX(1D0,-EPS)
       CTW = 4*AMW**2/AMH**2*DCMPLX(1D0,-EPS)
...
c      CLC = 4*XRMC**2/AMZ**2*DCMPLX(1D0,-EPS)
       ...
       ! CHANGED LINA
C           CLS = 4*AMS**2/AMZ**2*DCMPLX(1D0,-EPS)
C           CLU = 4*AMU**2/AMZ**2*DCMPLX(1D0,-EPS)
C           CLD = 4*AMD**2/AMZ**2*DCMPLX(1D0,-EPS)
       ! END LINA CHANGED
       ...
       !CHANGED LINA
C              CAS = FS*(CI1(CTS,CLS) - CI2(CTS,CLS))
C              CAU = FU*(CI1(CTU,CLU) - CI2(CTU,CLU))
C              CAD = FD*(CI1(CTD,CLD) - CI2(CTD,CLD))
       ! END LINA CHANGED
...
C
     ...
     .    +HNUPNUP+HEPEP+HBPBP+HTPTP+HUU+HDD
     .    +HEE
      PRINT *,'------------------'
      PRINT *,'HIGGS TOTAL WIDTH', WTOT, 'GeV'
      PRINT *,'------------------'
...
c     write(6,*)HBB,HWW,HZZ
      ...
      PRINT *,'BR(H -> dd) =' , SMBRD
      PRINT *,'----------------'
      PRINT *,'BR(H -> tau tau) =' , SMBRL
      PRINT *,'BR(H -> mu mu) =' , SMBRM
      PRINT *,'BR(H -> e e) =' , HEE/WTOT
      PRINT *,'----------------'
      PRINT *,'BR(H -> g g) =' , SMBRG
      PRINT *,'BR(H -> gamma gamma) =' , SMBRGA
      PRINT *,'BR(H -> gamma Z) =' , SMBRZGA
      PRINT *,'--- off shell ---'
      PRINT *,'BR(H -> W W) =' , SMBRW
      PRINT *,'BR(H -> Z Z) =' , SMBRZ
      PRINT *,'----------------'
      PRINT *,'2* BR(gamma gamma) * BR (bb) =' , 2d0*SMBRGA*SMBRB
      PRINT *,'2* BR(bb) * BR (bb) =' , 2d0*SMBRB*SMBRB
      PRINT *,'----------------'
c     write(6,*)HLL,HMM
...
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
c     write(53,*)AMCH,HSC
c     write(6,*)'H+ -> cs: ',AMCH,HSC,QSUSY1,LOOP
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
